...
{
  global $jq_form_next_id;
  $options = _parse_attributes($options);
  $options_html = _parse_attributes($options_html);
  // TBB: vanilla forms don't absolutely have to have IDs, but
  // we need one for jQuery's bind()
  if (!isset($options_html['id']))
  {
    if (!isset($jq_form_next_id))
    {
      $jq_form_next_id = 0;
    }
    $jq_form_next_id++; 
    $options_html['id'] = "jq-form-remote-tag-$jq_form_next_id";
  }
  $options['form'] = true;

  // TBB: we do this with a bind() instead so that it can be easily
  // invoked by either a real submit button or a jQuery (NOT vanilla)
  // submit() call
  ...
  $tag = tag('form', $options_html, true);
  $id = $options_html['id'];
  $function = jq_remote_function($options);
  $function = "jQuery.ajax({type:'POST',dataType:'html',data:jQuery(this).serialize(),success:function(data, textStatus){jQuery('#pk-context-cms-contents-body').html(data);},url:'/frontend_dev.php/cms/pkContextCMS/addSlot?name=body&id=1'})";
  $tag .= <<<EOM
<script>
$(document).ready($("#$id").bind("submit", 
...
function jq_sortable_element($selector, $options = array())
{
  ...
  jq_add_plugin(sfConfig::get('jquery_sortable', 
    'jquery-ui-sortable-1.6rc2.min.js'));
  $options = _parse_attributes($options);
  $url = json_encode(url_for($options['url']));
  unset($options['url']);
  $handleOptions = '';
  // Accept a selector for use as the handle. Not actually as flexible as
  // just writing jQuery code, which we should probable move to in the
  // future.
  if (isset($options['handle']))
  {
    $handleOptions = "handle: $('" . $options['handle'] . "')";
